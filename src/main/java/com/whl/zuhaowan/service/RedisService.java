...
import com.whl.zuhaowan.contants.CacheConstant;
...
//******************hash数据类型*********************
     ...
     * 获取键值延长缓存时间
     **/
    public boolean expire(String redisKey, final CacheConstant.UnitEnum unitEnum) {
        try {
            if (!isEmpty(redisKey)) {
                BoundValueOperations operations = redisTemplate.boundValueOps(redisKey);
                return operations.expire(unitEnum.getStortageTime(), unitEnum.getTimeUnit());
            }
        } catch (Throwable e) {
            log.error("getExprise key:" + redisKey + "  error:", e);
        }
        return false;
    }
    ...
    /**
     * @ModifyTime 2021/08/25
     * 判断对象是否为空 null -> true "" -> true " " -> true "null" -> true empty Collection -> true empty Array -> true
     * others -> false
     **/
    private boolean isEmpty(Object object) {
        if (object == null) {
            ...
            return "".equals(((String) object).trim()) || "null".equals(((String) object).trim());
